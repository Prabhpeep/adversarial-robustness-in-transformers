...
# ADDED return_attn for calculating Weirstrass distance and to visualise attention probabilities
    ...
    def forward(self, x, mask=None, return_attn=False):
            ...
            # Drop is only for values, we want raw probs for regularization
            attn_out = attn if return_attn else None
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, return_attn=False):
      ...
      # --- MODIFIED: Capture Attention ---
        if return_attn:
            attn_windows, attn_weights = self.attn(x_windows, mask=self.attn_mask, return_attn=True)
        else:
...
class BasicLayer(nn.Module):
    ...
    # In BasicLayer class for returning attention
    def forward(self, x, return_attn=False):
        attn_list = []
